...
const CalendarView = props => {
    ...
    function handleCellRender(value){
      ...
      // console.log(value);
      return (
        <Badge status={displayBadge} offset={[4,0]}/>
      )
    }
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} sm={12} md={12}>
            <Paper className={classes.paper}>
              <div style={{display: 'flex', justifyContent: 'center'}}>
                <Calendar fullscreen={false} dateCellRender={handleCellRender} onSelect={handleClickOpen} onPanelChange={onPanelChange} />
                <SimpleDialog 
                  open={open} 
                  onClose={handleClose} 
...
function SimpleDialog(props) {
  const classes = useStyles();
  const { onClose, open, selectedDay, selectedMonth, auditData, instData, outletData, setSelectedAudit} = props;
  ...
  return (
    <Dialog 
      ...
      <DialogTitle id="simple-dialog-title">{monthStr[selectedMonth]} {selectedDay}</DialogTitle>
...
function RenderDialogList(props){
  ...
  const {selectedMonth, selectedDay, auditData, instData, outletData, setSelectedAudit} = props;
  // console.log(auditData);
  var auditFiltered = [];
  for (var i = 0; i < auditData.length; i++){
    var deadline = auditData[i].deadline.split(" ");
  ...
  if (auditFiltered.length == 0){
    return (
      <Typography variant="subtitle1" align="center" style={{paddingBottom: 25}}>No audit retification due on this day.</Typography>
    )
  }else{
    return(
      <List component="nav" >
        {auditFiltered.map(item =>(
          <ListItem button divider={true} onClick={() => handleItemClick(item.id)}>
            <ListItemIcon>
              <ErrorOutlineIcon/>
            </ListItemIcon>
            <Typography component="div">
              ...
              <Box fontWeight="fontWeightMedium" m={1}>
                {getOutletAndInstitute(item.outlet_id, instData, outletData)[0]}, {getOutletAndInstitute(item.outlet_id, instData, outletData)[1]}
              </Box>  
              <Box fontWeight="fontWeightLight" fontStyle="normal" m={1}>
                {item.type} Form
              </Box>
              <Box fontStyle="italic" m={1}>
                {item.numberNC} NCs outstanding.
              </Box>
              </Grid>
            </Typography>
          </ListItem>
